...
def append_usadel_supercurrent_table_to_npz(
    ...
    *,
    ...
    This is intentionally strict: the PRE catalogue must expose or allow a
    deterministic reconstruction of the ``q`` and ``Delta`` axes.  The SS stage
    should never fabricate a Usadel current table.
    """
...
def load_usadel_current_diagnostic_catalog(usadel_npz: str | Path) -> dict[str, np.ndarray]:
    """Load only the arrays required by SS Usadel-current diagnostics."""
    usadel_npz = Path(usadel_npz)
    with np.load(usadel_npz, allow_pickle=False) as data:
        arrays = {key: data[key] for key in data.files}
...
def compute_dirty_usadel_supercurrent_table(
    *,
    ...
    omega = (2 * np.arange(n_matsubara, dtype=float) + 1.0) * np.pi * KB_J_K * float(T_K)
    ...
    for idelta, delta in enumerate(delta_axis):
        ...
        for iq, q in enumerate(q_axis):
            ...
            table[idelta, iq] = prefactor * q * float(np.sum(sin_theta * sin_theta))
    return table
