...
3. Do the electronic energy and heat-capacity tables behave smoothly enough for runtime interpolation?
"""
...
def plot_power_channels_Te_Tph_maps(
    ...
    *,
    ...
    cbar.set_label(r"power density [W m$^{-3}$], positive: electrons -> phonons")
...
def plot_energy_heat_capacity_curves(
    ...
    *,
    ...
    """Plot electronic energy and heat capacity for representative state slices."""
    output = _prepare_output(output_path)
    states = _representative_state_indices(catalog)

    fig, (ax_u, ax_c) = plt.subplots(1, 2, figsize=(11.2, 4.25), constrained_layout=True)
    for label, i_delta, i_q in states:
        ax_u.plot(catalog.Te_values_K, catalog.u_e_J_m3[:, i_delta, i_q], marker=".", markersize=2.0, linewidth=1.0, label=label)
        ax_c.plot(catalog.Te_values_K, catalog.C_e_J_m3_K[:, i_delta, i_q], marker=".", markersize=2.0, linewidth=1.0, label=label)

    ax_u.set_title(r"Electronic energy density")
    ax_u.set_xlabel(r"$T_e$ [K]")
    ax_u.set_ylabel(r"$u_e^S$ [J m$^{-3}$]")
    ax_u.grid(True, linewidth=0.35, alpha=0.28)

    ax_c.set_title(r"Effective electronic heat capacity")
    ax_c.set_xlabel(r"$T_e$ [K]")
    ax_c.set_ylabel(r"$C_e^S$ [J m$^{-3}$ K$^{-1}$]")
    ax_c.set_yscale("symlog", linthresh=max(1.0e-3, 1.0e-4 * float(np.nanmax(np.abs(catalog.C_e_J_m3_K)))))
    ax_c.grid(True, linewidth=0.35, alpha=0.28)
    ax_c.legend(loc="best", fontsize=7.2)

    fig.suptitle("Runtime energy tables extracted from the PRE power catalogue")
    fig.savefig(output, dpi=dpi, bbox_inches="tight")
    plt.close(fig)
    return output
