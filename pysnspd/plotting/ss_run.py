...
def plot_ss_edge_current_zoom_snapshots(
    ...
    *,
    ...
    four columns: a top insulating-edge window, a left terminal-edge window,
    a bottom-right corner window, and an interior strip window. The upper row
    shows the signed supercurrent edge projection ``j_s,e/j_avg``; the lower row
    shows the signed normal-current edge projection ``j_n,e/j_avg``.
...
def _edge_zoom_regions(
    ...
    """Return the standard local edge-current diagnostic regions."""
...
def _edge_zoom_highlight_edge(
    *,
    ...
    if kind == "top":
        ...
        # Interior strip: select an edge near the middle of the zoom window,
        # with a weak preference for transport-aligned edges.  This keeps the
        # highlighted edge away from boundary/corner artifacts.
        score = 0.35 * np.abs(dx) / ell
        score -= 0.02 * ((mx - center_x) ** 2 + (my - center_y) ** 2) ** 0.5
...
def _plot_one_edge_current_zoom_snapshot(
    *,
    ...
    fig, axes = plt.subplots(
        2,
        n_regions,
        figsize=(4.45 * n_regions + 1.0, 6.4),
        squeeze=False,
        constrained_layout=False,
    )
    fig.subplots_adjust(left=0.050, right=0.925, bottom=0.090, top=0.875, wspace=0.25, hspace=0.30)
