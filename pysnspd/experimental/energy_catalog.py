...
def retarded_spectrum(energy: np.ndarray | float, *, delta: float,
    ...
    for index in np.ndindex(energies.shape):
        ...
        if gamma == 0:
            ...
            # Nearly degenerate polynomial roots at very small Gamma lose
            # digits. Refine the causal candidate in the ORIGINAL equation,
            # accepting a Newton step only when its residual decreases.
            for _ in range(12):
                w=np.sqrt(u*u-delta*delta)
                if w.imag<0:
                    w=-w
                residual_u=u-z-1j*gamma*u/w
                if abs(residual_u)<2e-14*max(1,abs(u),delta,gamma):
                    break
                derivative_u=1+1j*gamma*delta**2/w**3
                if abs(derivative_u)==0:
                    break
                step=residual_u/derivative_u
                accepted=False
                for damping in (1,.5,.25,.125,.0625,.03125):
                    candidate=u-damping*step
                    if candidate.imag<=0:
                        continue
                    candidate_w=np.sqrt(candidate*candidate-delta*delta)
                    if candidate_w.imag<0:
                        candidate_w=-candidate_w
                    candidate_residual=candidate-z-1j*gamma*candidate/candidate_w
                    if abs(candidate_residual)<abs(residual_u):
                        u=candidate
                        accepted=True
                        break
                if not accepted:
...
class OccupationEnergyCatalog:
    ...
    def __post_init__(self) -> None:
        ...
        value_coefficients=CubicSpline(self.vacuum.delta_axis,np.log(increments),axis=0).c
        slope_coefficients=CubicSpline(self.vacuum.delta_axis,gamma_increments/increments,axis=0).c
    ...
    def energy_kernel(self, delta: float, gamma: float) -> tuple[np.ndarray,np.ndarray,np.ndarray]:
        ...
        j=min(max(int(np.searchsorted(self.vacuum.gamma_axis,gamma,side='right')-1),0),len(self.vacuum.gamma_axis)-2)
        ...
        h=self.vacuum.gamma_axis[j+1]-self.vacuum.gamma_axis[j]
        t=(gamma-self.vacuum.gamma_axis[j])/h
        def hermite(v,m):
            return ((2*t**3-3*t*t+1)*v[0]+(t**3-2*t*t+t)*h*m[0]+
                    (-2*t**3+3*t*t)*v[1]+(t**3-t*t)*h*m[1])
        log_g=((6*t*t-6*t)*values[0]/h+(3*t*t-4*t+1)*slopes[0]+
               (-6*t*t+6*t)*values[1]/h+(3*t*t-2*t)*slopes[1])
        logs=(hermite(values,slopes),hermite(values_d,slopes_d),log_g)
    ...
    def save(self,path: str | Path) -> Path:
        path=Path(path)
        path.parent.mkdir(parents=True,exist_ok=True)
        metadata=dict(self.vacuum.metadata, schema="pysnspd.experimental.occupation_catalog.v2",
                      ...
                      interpolation_policy="one Uvac spline; log positive E increments: cubic in Delta, Hermite in Gamma with exact nodal spectral slopes; all forces differentiated")
        with path.open("wb") as stream:
    ...
    def load(cls,path: str | Path) -> "OccupationEnergyCatalog":
        with np.load(path,allow_pickle=False) as values:
            metadata=json.loads(str(values["metadata_json"]))
            if metadata.get("schema")!="pysnspd.experimental.occupation_catalog.v2":
...
def build_occupation_catalog(vacuum: UniformVacuumCatalog, *, count_order: int=48,
    ...
    energies,gamma_derivatives=[],[]
    for delta in vacuum.delta_axis:
        energy_row,derivative_row=[],[]
        for gamma in vacuum.gamma_axis:
            energy=energy_at_count(count,delta=delta,gamma=gamma,eta=eta)
            c,s=retarded_spectrum(energy,delta=delta,gamma=gamma,eta=eta)
