...
def refined_count_catalog(base, refinement=1, *, bulk_spacing=.02, order=2,
    ...
    with the refinement level, as are the bulk and tail intervals. A local .005
    count step through x=.1 also resolves the specified near-edge occupation
    band centered at .035 with width .01. No force or population is repaired
    after integration.
    """
    ...
    pieces = [np.array([.01])]
    for low, high, step in ((.01, .1, min(.005, bulk_spacing)),
                            (.1, 3., bulk_spacing), (3., 6., .05), (6., 12., .15)):
