...
class ProjectedElectronPhononEvents(ElectronPhononEvents):
    ...
    def from_arrays(cls, electron_energies, electron_weights, count_nodes, coherence_ratio,
                    ...
                    spectral_parameters: tuple[float, float, float] | None = None):
        return cls(electron_energies, electron_weights, count_nodes, coherence_ratio, phonons,
                   ...
                   max_energy_panel=max_energy_panel, spectral_parameters=spectral_parameters)
    ...
    def from_cell(cls, cell, phonons: PhononGrid, *, rate_prefactor: float,
        ...
        return cls(cell.energies, cell.weights, cell.catalog.count_nodes, s.imag/c.real,
                   ...
                   spectral_parameters=(cell.amplitude, cell.gamma, cell.catalog.eta))
    ...
    def __init__(self, electron_energies, electron_weights, count_nodes, coherence_ratio,
        ...
        exact_bcs = False
        if spectral_parameters is not None:
            spectral_parameters = np.asarray(spectral_parameters, dtype=float)
            if (spectral_parameters.shape != (3,) or np.any(~np.isfinite(spectral_parameters))
                    or np.any(spectral_parameters[:2] < 0) or spectral_parameters[2] <= 0):
                raise ValueError("spectral parameters must be finite nonnegative delta/Gamma and positive eta")
            amplitude, gamma, eta = spectral_parameters
            exact_bcs = gamma == 0

        def energy_at(values):
            if exact_bcs:
                return values*np.sqrt(1+amplitude**2/(values**2+eta**2))
            return np.interp(values, counts, energy)

        def count_at(values):
            if exact_bcs:
                result = np.real(np.sqrt((np.asarray(values)+1j*eta)**2-amplitude**2))
                # Preserve exact known support endpoints through inverse roundoff.
                return np.where(values == emin, xmin, np.where(values == emax, xmax, result))
            return np.interp(values, energy, counts)

        def ratio_at(values):
            if exact_bcs:
                denominator = (values**2+eta**2)
                return values*amplitude/np.sqrt(denominator*(denominator+amplitude**2))
            return np.interp(values, counts, self.coherence_ratio)
        ...
        for rec in (False, True):
            ...
            if outer_max_energy <= emin:
                continue
            outer_max_count = count_at(outer_max_energy)
            ...
            low = count_at(inner_low_energy[valid])
            high = count_at(inner_high_energy[valid])
            if integration_layout != "global":
                if integration_layout == "resolved_panels":
                    ...
                    e_start = energy_at(count_start)
                    e_end = energy_at(count_end)
                    ...
                    if exact_bcs:
                        starts = count_at(energy_left-ei[owner] if rec else energy_left+ei[owner])
                        ends = count_at(energy_right-ei[owner] if rec else energy_right+ei[owner])
                        span = ends-starts
                    else:
                        # Within one linear E(x) segment, the intersection is exact.
                ...
                if integration_layout == "energy_panels":
                    ...
                    starts = count_at(energy_start)
                    ends = count_at(energy_end)
                    span = ends-starts
                ...
                r_i = np.broadcast_to(ratio_at(outer)[owner, None], inner.shape).ravel()
                ...
                r_i = np.broadcast_to(ratio_at(outer)[:, None], inner.shape).ravel()
                pair_weight = (outer_weight[:, None]*inner_weight).ravel()
            e_j = energy_at(inner.ravel())
            r_j = ratio_at(inner.ravel())
        ...
        self.metadata = {
            ...
            "electron_reconstruction": "exact causal finite-eta BCS E(x), x(E), R2/N1" if exact_bcs else "piecewise-linear E(x) and R2/N1 on native count nodes; no extrapolation",
