...
def save_stationary_state_npz(state: GTDGLStationaryState, output_path: str | Path) -> Path:
    ...
    circuit = dict(state.metadata.get("circuit_runtime", {}) or {})
    circuit_state = dict(circuit.get("final_state", {}) or {})
    circuit_params = dict(circuit.get("params", {}) or {})
    for key, value in circuit_state.items():
        payload[f"circuit_{key}"] = np.asarray(float(value))
    for key, value in circuit_params.items():
        if value is not None:
            payload[f"circuit_param_{key}"] = np.asarray(float(value))
