...
def test_box_matches_pytdgl_rectangle_point_count_distribution():
    coords = box(width=2.4e-7, height=1.2e-7, points=101, center=(1.2e-7, 0.0))
...
def test_rectangular_boundary_points_uses_box_coordinate_convention():
    coords = rectangular_boundary_points(2.4e-7, 1.2e-7, points=101)
    expected = box(width=2.4e-7, height=1.2e-7, points=101, center=(1.2e-7, 0.0))
...
def test_minimal_pytdgl_device_copy_and_contains_points():
    film = Polygon(
        "film",
        points=rectangular_boundary_points(2.4e-7, 1.2e-7, points=101),
    )
    device = Device(
        "test",
        layer=Layer(london_lambda=1.0, coherence_length=1.0, thickness=1.0),
        film=film,
        length_units="m",
    )

    points = np.array([[1.2e-7, 0.0], [4.0e-7, 0.0]])
