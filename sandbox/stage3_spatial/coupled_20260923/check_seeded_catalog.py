...
def main():
    ...
    p.add_argument('--registration',default='docs/implementation/stage3/coupled_20260923/seeded_registration.json')
    ...
    files=['pysnspd/experimental/'+n+'.py' for n in ['local_energy_patch','seeded_count_catalog','refined_cells','energy_catalog','spatial_functional']]
    files+=['sandbox/stage3_spatial/coupled_20260923/check_seeded_catalog.py',args.registration,reg['catalogue'],reg['predictor']]
    ...
    try:
        ...
        for degree in reg['degrees']:
            ...
            artifact=ROOT/reg['predictor']
            if sha(artifact)!=reg['predictor_sha256']:raise ValueError('predictor artifact changed')
            old=json.loads((ROOT/reg['predictor_manifest']).read_text())
            for filename,expected in old['sources'].items():
                if sha(ROOT/filename)!=expected:raise ValueError('predictor source changed: '+filename)
            predictor=LocalEnergyPatch.load(artifact,source)
            patch=SeededCountCatalog(source,predictor)
            shutil.copy2(artifact,output/f'patch_degree{degree}.npz');patches[degree]=patch
        ...
        result=dict(status='PASS_SEEDED_CAUSAL_CATALOG' if passed else 'SEEDED_CAUSAL_CATALOG_NOT_ACCEPTED',
            ...
            speedup_scope='Corrected causal kernel only, not whole RHS or trajectory speedup.',
            representation='predictor_then_exact_causal_Newton',
