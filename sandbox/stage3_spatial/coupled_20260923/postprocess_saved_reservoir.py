...
def allocation(model, archived_mass, archived_mapping):
    value = np.zeros((model.quadrature_size, len(model.graph_edges)))
    node_mass = np.bincount(archived_mapping, weights=archived_mass, minlength=model.cells)
    for edge, ends in enumerate(model.graph_edges):
        for node in ends:
            rows = np.flatnonzero(archived_mapping == node)
            value[rows, edge] += .5*archived_mass[rows]/node_mass[node]
    return value
...
def postprocess_case(input_root, original, oldreg, reg, output):
    ...
    geometry_mass_roundoff = float(np.max(abs(model.quadrature_mass_bar-mass)/mass))
    if geometry_mass_roundoff > 64*np.finfo(float).eps:
        raise ValueError("reassembled mass differs beyond floating point geometry roundoff")
    ...
    normal = normal_heat_distribution(conductance, potential.delta_phi_V, allocation(model, mass, mapping))
    ...
    result = dict(case_id=name, status="PASS_SAVED_RESERVOIR_INSTANTANEOUS_IDENTITIES" if passed else "SAVED_RESERVOIR_NOT_ACCEPTED",
        ...
        recovery=dict(archived_deposited_rate_bar=original_heat, reconstructed_original_deposited_rate_bar=recovered_old_heat,
                      geometry_mass_relative_roundoff=geometry_mass_roundoff, mass_used="EXACT_ARCHIVED_QUADRATURE_MASS",
