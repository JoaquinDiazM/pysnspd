...
def registration():
    ...
    return dict(schema='pysnspd.stage2.special-short-cases.registration.v1',
        ...
        limitations=['No time-refinement certificate for these short special trajectories.',
            'Finite-Gamma equilibrium upper support is assessed separately by the root reviewer; no certificate is inferred here.',
            'No global stage2 or production admission follows from this block.'],
        execution=dict(expected_seconds=[90, 150], bounded_timeout_seconds=240,
...
def execute(plan,path,output):
    ...
    try:
        ...
        check_sources(plan)
        report['status']='PASS_SHORT_SPECIAL_SCENARIOS_EQUILIBRIUM_SUPPORT_EXTERNAL'
        report['remaining']=['Finite-Gamma equilibrium upper support is assigned to a separate independent postprocessor.', 'These cases are not a three-level temporal refinement or a global stage2 closure.']
