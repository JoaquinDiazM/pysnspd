"""Manual foreground continuation for registered SSP population-mesh checks.
...
def validate_plan(plan,executing=False):
    if plan.get('schema')!='pysnspd.stage2.remaining-mesh-plan.v1':
    ...
    for task in plan.get('tasks',[]):
        ...
        if kind in ('trajectory','existing_reference'):
            ...
            if parameters['case'] != 'two' or parameters['scenario'] not in (
            ...
            elif parameters['steps'] not in (320,640):
                raise ValueError('New mesh trajectories must use the registered320/640step controls')
...
def validate_trajectory(path,parameters,contracts):
    ...
    sys.path[:0]=[str(ROOT/'sandbox/stage2_cells'),str(ROOT/'sandbox/stage2_cells/recovery_20260921')]
    from assess_grid_refinement import read
    from assess_limited_time import verify_contract
...
def self_test():
    ...
    plan=json.loads((ROOT/'docs/implementation/stage2/closure_prep_20260922/remaining_mesh_plan.json').read_text())
    ...
    invalid=copy.deepcopy(plan)
    next(item for item in invalid['tasks'] if item['kind']=='trajectory')['parameters']['case']='one'
    try:validate_plan(invalid,False)
    except ValueError:checks['one_cell_case_cannot_enter_two_cell_plan']=True
    else:checks['one_cell_case_cannot_enter_two_cell_plan']=False
    invalid=copy.deepcopy(plan)
    next(item for item in invalid['tasks'] if item['kind']=='limited_pair')['accepted_statuses']=['PASS']
    try:validate_plan(invalid,False)
    except ValueError:checks['supplementary_pair_does_not_accept_global_PASS_status']=True
    else:checks['supplementary_pair_does_not_accept_global_PASS_status']=False
    references=[item for item in plan['tasks'] if item['kind']=='existing_reference']
    checks['all_four_archived_SSP_references_are_registered']=(len(references)==4
        and sorted(item['parameters']['steps'] for item in references)==[160,320,640,1280]
        and all(item['parameters']['method']==METHOD for item in references))
    invalid=copy.deepcopy(plan)
    next(item for item in invalid['tasks'] if item['kind']=='existing_reference')['parameters']['method']='rk4'
    try:validate_plan(invalid,False)
    except ValueError:checks['archived_reference_cannot_silently_switch_integrator']=True
    else:checks['archived_reference_cannot_silently_switch_integrator']=False
    invalid=copy.deepcopy(plan)
    grid_index=next(i for i,item in enumerate(invalid['tasks']) if item['kind']=='grid')
    grid=invalid['tasks'][grid_index]
    original_input=next(item for item in invalid['tasks'] if item['id']==grid['runs'][0])
    extra=copy.deepcopy(original_input);extra.update(id='mismatched_grid_time',kind='trajectory')
    extra.pop('path',None);extra['parameters']['steps']=640
    grid['runs'][0]=extra['id'];invalid['tasks'].insert(grid_index,extra)
    try:validate_plan(invalid,False)
    ...
    sys.path[:0]=[str(ROOT/'sandbox/stage2_cells'),str(ROOT/'sandbox/stage2_cells/recovery_20260921')]
    from assess_limited_time import verify_contract
    reference_record=json.loads(workspace_path(references[0]['path']).read_text())
    verify_contract({'record':reference_record})
    checks['real_completed_SSP_reference_integration_contract_verified']=True
    invalid_record=copy.deepcopy(reference_record)
    invalid_record['time_integration']['stats']['status']='FAILED'
    try:verify_contract({'record':invalid_record})
    except ValueError:checks['failed_SSP_integration_stats_cannot_be_reused']=True
    else:checks['failed_SSP_integration_stats_cannot_be_reused']=False
    temporary_parent=ROOT/'tmp';temporary_parent.mkdir(exist_ok=True)
    with tempfile.TemporaryDirectory(prefix='mesh_batch_guards_',dir=temporary_parent) as folder:
        testroot=Path(folder)
        dry_output=testroot/'dry_output'
        args=['batch','--plan',str(ROOT/'docs/implementation/stage2/closure_prep_20260922/remaining_mesh_plan.json'),
        ...
        pair=dict(id='pair',phase='fixture',kind='limited_pair',runs=['first','first'],
                  accepted_statuses=['PASS_PAIR_DIAGNOSTIC'])
        def pair_pass(command,log):write_new(Path(command[-1]),dict(status='PASS_PAIR_DIAGNOSTIC'))
        output,calls,failure=invoke('pair_only',[first,pair],pair_pass)
        checks['supplementary_pair_remains_distinct_and_never_issues_stage_admission']=(
            calls==1 and failure is None
            and json.loads((output/'pair.json').read_text())['status']=='PASS_PAIR_DIAGNOSTIC'
            and '"stage2_admission": false' in (output/'batch_events.jsonl').read_text())
    return dict(schema='pysnspd.stage2.remaining-mesh-offline-selftest.v1',
        status='PASS' if all(checks.values()) else 'FAIL',checks=checks,
        scope='Synthetic orchestration guards and read-only archived SSP metadata; no subprocess, physical RHS or time integration.',
...
def main():
    ...
    signature=dict(schema='pysnspd.stage2.remaining-mesh-manifest.v1',plan_sha256=sha(plan_path),
