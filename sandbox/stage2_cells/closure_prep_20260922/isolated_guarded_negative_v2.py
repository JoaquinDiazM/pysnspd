"""Corrected reverse-channel fixture; prior zero-activity precheck is retained."""
...
ROOT=base.ROOT
OUT=ROOT/'docs/implementation/stage2/closure_prep_20260922/isolated_guarded_negative_v2'
...
def registration():
    return dict(schema='pysnspd.stage2.guarded-negative-channel-registration.v2',
        status='PREREGISTERED_NOT_EXECUTED',source_hashes=sources(),
        reason='The first supplemental fixture was rejected before integration because isolated nonzero electronic nodes have zero geometric interval activity. Use positive interior absorption populations and place the subnormal phonon at Omega=.7, active in both reverse channels.',
        ...
        cases=dict(absorption=dict(p=[.2,.1,.01,.001],family='scattering',direction='backward'),
                   creation=dict(p=[0.,0.,0.,0.],family='recombination',direction='backward')),
        common=dict(amplitude=0.,gamma=0.,n=[.05,.05,1e-320,.05,.05,.05],
...
def main():
    ...
    output=dict(schema='pysnspd.stage2.guarded-negative-channel-results.v2',status='INCOMPLETE',
    ...
    try:
        ...
        for name,case in registration()['cases'].items():
            ...
            p=np.array(case['p']);n=np.array([.05,.05,1e-320,.05,.05,.05])
            ...
            prechecks=dict(initial_guard_active=guarded.needs_extended(system,initial),
                           ...
                           raw_phonon_consumption=bool(np.dot(system.phonons.capacities,dn)<0), subnormal_phonon_has_raw_consumption=bool(dn[2]<0))
            ...
            checks=dict(prechecks,
                ...
                net_phonon_consumption=record['final']['phonon_count']<record['initial']['phonon_count'], subnormal_phonon_consumed=bool(final_n[0,2]<n[2]),
            ...
            output['cases'].append(dict(case=name,checks=checks,energy_error=record['energy_error'],
                ...
                subnormal_phonon_index=2,subnormal_phonon_energy=.7,subnormal_phonon_initial=float(n[2]),subnormal_phonon_final=float(final_n[0,2])))
