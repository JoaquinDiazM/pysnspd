"""Foreground, logged recovery batch for the explicitly registered SSP experiment.

Default is a dry run. No retry, evidence overwrite, scheduler, or background
process is started here. A controlling screen session may detach externally.
"""
...
def validate_plan(plan,executing=False):
    if plan.get('schema')!='pysnspd.stage2.limited-recovery-plan.v1':
        raise ValueError('Unsupported recovery-plan schema')
    if executing and plan.get('status')!='READY_FOR_AUTHORIZED_SCREEN_RUN':
...
def self_test():
    ...
    plan=json.loads((ROOT/'docs/implementation/stage2/recovery_20260921/limited_acceptance_plan.json').read_text())
    ...
    pair=next(item for item in invalid['tasks'] if item['kind']=='limited_pair')
    pair['accepted_statuses']=['PASS']
    try:validate_plan(invalid,False)
    except ValueError:checks['paired_diagnostic_cannot_be_registered_as_global_PASS']=True
    else:checks['paired_diagnostic_cannot_be_registered_as_global_PASS']=False
    ...
    with tempfile.TemporaryDirectory(prefix='limited_batch_guards_',dir=temporary_parent) as folder:
        ...
        args=['batch','--plan',str(ROOT/'docs/implementation/stage2/recovery_20260921/limited_acceptance_plan.json'),
        ...
        diagnostic=dict(id='pair',phase='fixture',kind='limited_pair',runs=['first','first'],
        ...
        output,calls,failure=invoke('pair_only',[first,diagnostic],pair_pass)
        checks['pair_result_keeps_distinct_status_and_does_not_issue_stage_admission']=(
            calls==1 and failure is None and json.loads((output/'pair.json').read_text())['status']=='PASS_PAIR_DIAGNOSTIC'
            and '"stage2_admission": false' in (output/'batch_events.jsonl').read_text())
    return dict(schema='pysnspd.stage2.limited-batch-offline-selftest.v1',
...
def main():
    ...
    signature=dict(schema='pysnspd.stage2.limited-recovery-manifest.v1',plan_sha256=sha(plan_path),
