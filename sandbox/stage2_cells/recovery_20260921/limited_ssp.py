...
def _ratio(available, demand, h):
    ...
    # A continuous ramp: the safety margin reaches one at raw=1/SAFETY.
    # Switching from SAFETY to one immediately above raw=1 creates a jump
    # in the finite-step map and spoils its temporal refinement.
    result[active] = np.minimum(1., SAFETY*raw[active])
