"""Recorded SSPRK3 experiment with shared conservative event-flux limiting."""
...
def main():
    ...
    with progress.open('x',encoding='utf-8',buffering=1) as stream:
        ...
        def replacement(rhs,initial,duration,steps):
            times,states,_=limited_ssp.integrate(rhs.__self__,initial,duration,steps,
        ...
        try:
            ...
            if output.exists():
                ...
                record['parameters']['method']='ssprk3_common_flux_limited'
                ...
                for source in (Path(__file__),Path(limited_ssp.__file__)):
