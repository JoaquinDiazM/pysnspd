"""Manual foreground diagnostic of two-cell SSP temporal convergence.
...
def validate_plan(plan,executing=False):
    if plan.get('schema')!='pysnspd.stage2.two-cell-time-plan.v1':
...
def self_test():
    ...
    plan=json.loads((ROOT/'docs/implementation/stage2/time_pass_20260922/two_cell_time_plan.json').read_text())
    ...
    with tempfile.TemporaryDirectory(prefix='two_cell_batch_guards_',dir=temporary_parent) as folder:
        testroot=Path(folder)
        dry_output=testroot/'dry_output'
        args=['batch','--plan',str(ROOT/'docs/implementation/stage2/time_pass_20260922/two_cell_time_plan.json'),
    ...
    return dict(schema='pysnspd.stage2.two-cell-time-offline-selftest.v1',
...
def main():
    ...
    signature=dict(schema='pysnspd.stage2.two-cell-time-manifest.v1',plan_sha256=sha(plan_path),
