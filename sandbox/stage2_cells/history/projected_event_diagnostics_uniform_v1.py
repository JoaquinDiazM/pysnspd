...
def run(orders, phonon_nodes, method, refinement=None, integration_layout="global", max_count_panel=.5):
    ...
    projection_reference = json.loads(PROJECTION_REFERENCE.read_text(encoding="utf-8"))
    rows = []
    for label, amplitude, gamma in (("normal", 0., 0.), ("gapped", .72, 0.), ("gapless", .35, .65)):
        ...
        for order in orders:
            ...
            network = constructor.from_cell(cell, phonon_grid(phonon_nodes),
...
def main():
    ...
    if not CRITERIA.exists() or not REFERENCE.exists() or not PROJECTION_REFERENCE.exists():
    ...
    for refinement in args.refinements or [None]:
        rows.extend(run(args.orders, args.phonon_nodes, args.method, refinement,
                        args.integration_layout, args.max_count_panel))
    convergence = []
    ...
    result = {"schema": "pysnspd.stage2.projected_event_diagnostics.v1", "host": platform.node(),
              "criteria_sha256": sha(CRITERIA), "catalog_sha256": sha(CATALOG), "reference_sha256": sha(REFERENCE),
              "projection_reference_sha256": sha(PROJECTION_REFERENCE),
