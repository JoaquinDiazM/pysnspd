...
def profile():
    ...
    return query,reference,curves,[len(mesh.count_nodes) for mesh in meshes]
...
def main():
    ...
    convergence.set(xlabel='Estados electrónicos por celda',ylabel='Error máximo de los seis casos [%]',title='a) Convergencia del transporte',ylim=(8e-5,1.4),xlim=(145,max(counts)*1.16))
    ...
    query,reference,curves,profile_counts=profile(); scale=float(np.max(reference))
    curve.semilogy(query,reference/scale,color='black',lw=2,label='Referencia continua')
    for values,color,style,label in zip(curves,['#a64e36','#226e96'],['--',':'],[f'R2: {profile_counts[0]} estados',f'Complementario: {profile_counts[1]}']):
    ...
    local_label=f'{local_max:.4f}'.replace('.',',')
    residual.text(.98,.06,f'{profile_counts[1]}: máx. {local_label} %',transform=residual.transAxes,
    ...
    curves_path.write_text(json.dumps({'schema':'pysnspd.stage2.transport-profile.v1',
        ...
        'complementary_states':profile_counts[1],
        'complementary_flux_density':curves[1].tolist(),
        'complementary_maximum_local_relative_error_percent':local_max,
