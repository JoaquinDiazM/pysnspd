...
from pysnspd.gtdgl.pytdgl_like.usadel_current import attach_usadel_supercurrent_table_from_npz
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--ss-progress", action="store_true", help="Show a progress bar during the pyTDGL-like solve loop.")
    parser.add_argument(
        "--ss-supercurrent-law",
        choices=["gl", "usadel-poisson"],
        default="gl",
        help="Supercurrent used in the Poisson projection: native GL or diagnostic Usadel table.",
    )
...
def main() -> int:
    ...
    usadel_catalog = attach_usadel_supercurrent_table_from_npz(
        load_usadel_catalog_npz(usadel_path),
        usadel_path,
    )
    ...
    result = solve_stationary_pytdgl_like(
        ...
        supercurrent_law=args.ss_supercurrent_law,
    )
