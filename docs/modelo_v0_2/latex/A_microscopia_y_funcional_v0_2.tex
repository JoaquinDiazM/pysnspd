...
\section{A.0. Propósito de esta
revisión}\label{a.0.-propuxf3sito-de-esta-revisiuxf3n}

Se busca ampliar las capacidades de modelamiento de SNSPD: describir
mejor cómo la corriente, el espectro electrónico y las excitaciones
intercambian energía, conservando una implementación que pueda
verificarse y utilizarse. La meta de esta etapa es un modelo
microscópicamente informado y controlable, con mejoras medibles frente a
la referencia de la memoria. No se exige reconstruir la dinámica
completa del material desde primeros principios para avanzar.

\textbf{Corrección de la revisión 0.1.} Usar una corriente Usadel en una
ecuación de orden GL modificada tiene una justificación explícita.
Vodolazov introduce en el lado derecho de su ecuación (36), p.~11, una
corrección proporcional a la diferencia de divergencias de las
corrientes Usadel y GL. Su función es obtener la conservación de la
corriente Usadel en el estado estacionario {[}V{]}. La memoria
desarrolla esa cancelación en su anexo C.2, ecuaciones (C.4)--(C.10),
pp.~impresas 121--123 {[}M{]}. La combinación es un cierre aproximado
motivado, cuya validez debe evaluarse mediante sus observables y su
régimen de uso. No corresponde descalificarla porque sus partes
provengan de distintas aproximaciones.

La pregunta nueva es más concreta: \textbf{¿podemos obtener la corriente
y la fuerza sobre \(|\Delta|\) a partir de una misma energía
electrónica, para facilitar el balance durante la evolución?} Tener ese
punto de partida proporciona identidades comprobables y evita contar dos
veces el superflujo. Es una mejora de organización física; no es, por sí
sola, una prueba de que toda predicción anterior sea incorrecta.

El recorrido del documento es: fijar convenciones (A.1), construir esa
energía sin imponer todavía el equilibrio de \(|\Delta|\) (A.2),
identificar el espectro que sienten las excitaciones (A.3), proponer el
acoplamiento no térmico adiabático (A.4), comparar cierres uniformes
(A.5) y separar las limitaciones materiales de las matemáticas
(A.6--A.7). Las hipótesis de la extensión no térmica se distinguen del
funcional de equilibrio establecido.
...
A.3 se resuelve para cada amplitud ensayada. A.4 selecciona después la
amplitud de equilibrio. \textbf{Resolver el espectro no obliga a imponer
instantáneamente el equilibrio del condensado durante un evento.} Esta
distinción es esencial para que \(|\Delta|\) pueda seguir siendo una
variable dinámica.
...
hasta
A.6}\label{a.2.2.-desde-la-expresiuxf3n-de-virtanen-vargunin-y-silaev-hasta-a.6}
...
\(\lambda\) de A.32. La traza abarca \textbf{Nambu y espín}, y la suma
...
resta del estado normal se hará antes de retirar el corte.
...
sumando de A.6a aporta dos veces su valor con \(n\geq0\). El estado
normal tiene \(c_n=\operatorname{sgn}\epsilon_n\) y \(s_n=0\). Sustituir
A.6c--A.6d y restarlo da
...
Insertar A.6f en A.6e reúne la divergencia en una diferencia
...
Hasta aquí la energía es una diferencia respecto del estado normal. Para
recuperar también su entropía y capacidad calorífica se añade
...
El cero de energía se toma en el metal normal a \(T=0\). La parte normal
da \(C_{e,n}=2\pi^2N_0k_B^2T/3\). Omitir \(f_n\) no altera la fuerza de
amplitud, pero sí elimina incorrectamente ese calor almacenado. El
superíndice FD significa que las ocupaciones son Fermi--Dirac; no
implica todavía que \(|\Delta|\) satisfaga A.4.
...
el pequeño rectángulo en el orden inverso. Un desacuerdo de A.10
...
corriente. La corrección de fase de Vodolazov hace que la ecuación
estacionaria utilice la divergencia de la corriente elegida. A.10
comprueba además una relación entre la fuerza de amplitud y la respuesta
de corriente. Para aplicarla al cierre de la memoria hay que identificar
primero su normalización de fuerza y posibles coeficientes cinéticos; la
sola procedencia GL/Usadel no decide el resultado.
...
\begin{figure}
\centering
\includegraphics[width=0.88\linewidth,height=\textheight,keepaspectratio,alt={Resultado uniforme: el borde espectral cae por debajo de la amplitud al aumentar la ruptura de pares. Esta curva adimensional ilustra A.14 y no constituye una predicción de latencia.}]{../figuras/A_03_gap_espectral.png}
...
ilustra A.14 y no constituye una predicción de latencia.}
\end{figure}
...
En el cálculo uniforme de esta revisión, el máximo de corriente Usadel
tiene \(|\Delta|\simeq1.04008\) meV y \(E_g\simeq0.428529\) meV: el
borde es aproximadamente el 41.2\% de la amplitud. Es un resultado del
catálogo uniforme, que identifica una región espectral omitida si se
impone el umbral BCS. Su impacto en la latencia requiere una
comprobación dinámica posterior.

\section{A.4. Acoplamiento no térmico al
condensado}\label{a.4.-acoplamiento-no-tuxe9rmico-al-condensado}

\subsection{A.4.1. Qué se propone y qué debe
comprobarse}\label{a.4.1.-quuxe9-se-propone-y-quuxe9-debe-comprobarse}
...
\subsection{A.4.2. Seguir estados cuando cambia su
energía}\label{a.4.2.-seguir-estados-cuando-cambia-su-energuxeda}
...
A.16. Estas igualdades se entienden con el regulador causal y, si el
borde es singular, después de integrar antes de tomar \(\eta\to0^+\).

Para las ramas simétricas, \(R_2(0)=N_2(0)R_2(0)=0\). Integrando A.16 y
derivando A.15 a \(x\) fijo,
...
\subsection{A.4.3. Energía y fuerzas de una población no
térmica}\label{a.4.3.-energuxeda-y-fuerzas-de-una-poblaciuxf3n-no-tuxe9rmica}
...
Variando A.18 a \(p(x)\) fijo y utilizando A.17 junto con
\(dx=\rho\,dE\),
...
fuerzas son derivadas del \textbf{modelo adiabático A.18}; no
...
es decir, Fermi--Dirac. Sobre esa población, A.21 recupera A.8. Para la
temperatura equivalente de energía \(T_E\) definida en B, la diferencia
de fuerza es
...
escalar con la normalización GL apropiada. Fuera de ese régimen, A.23
...
\subsection{A.4.4. Igual energía no implica igual acción sobre el
condensado}\label{a.4.4.-igual-energuxeda-no-implica-igual-acciuxf3n-sobre-el-condensado}
...
\begin{figure}
\centering
\includegraphics[width=0.94\linewidth,height=\textheight,keepaspectratio,alt={Prueba sintética compartida con B: redistribuir la misma energía entre excitaciones cambia el momento que actúa sobre el condensado. Las curvas se construyen para enseñar esta diferencia, sin simular una cascada del material.}]{../figuras/B_03_misma_energia.png}
\caption{Prueba sintética compartida con B: redistribuir la misma
energía entre excitaciones cambia el momento que actúa sobre el
condensado. Las curvas se construyen para enseñar esta diferencia, sin
simular una cascada del material.}
\end{figure}

Los paquetes de ancho finito reejecutados en v0.2 dan una razón de
fuerzas de aproximadamente \(25.19\). La diferencia respecto de \(25\)
proviene del ancho y la normalización de los paquetes. B detalla sus
distribuciones y momentos; este cociente no se identifica con un factor
de cambio en la latencia.

\section{A.5. Corriente uniforme: conservar la justificación y medir la
aproximación}\label{a.5.-corriente-uniforme-conservar-la-justificaciuxf3n-y-medir-la-aproximaciuxf3n}

\subsection{A.5.1. Qué hace la corrección de corriente de la
memoria}\label{a.5.1.-quuxe9-hace-la-correcciuxf3n-de-corriente-de-la-memoria}
...
fase. La corrección del RHS discutida en A.0 actúa sobre la tercera. El
...
\subsection{A.5.2. Rama de amplitud GL
modificada}\label{a.5.2.-rama-de-amplitud-gl-modificada}
...
\subsection{A.5.3. Corriente aproximada de
Vodolazov}\label{a.5.3.-corriente-aproximada-de-vodolazov}
...
\subsection{A.5.4. Corriente Usadel sobre dos ramas de
amplitud}\label{a.5.4.-corriente-usadel-sobre-dos-ramas-de-amplitud}

Para corriente Usadel y amplitud A.25, la regla de la cadena produce el
...
A.31 localiza la pérdida de estabilidad uniforme; bordes, constricciones
y entrada de vórtices pueden reducir la corriente de switching de un
dispositivo.

\begin{figure}
\centering
\includegraphics[width=0.94\linewidth,height=\textheight,keepaspectratio,alt={Comparación de cierres uniformes con las mismas escalas materiales. El máximo de cada rama prueba su equilibrio uniforme; no reproduce el switching de una geometría bidimensional ni la señal de un fotón.}]{../figuras/A_02_corriente_uniforme.png}
\caption{Comparación de cierres uniformes con las mismas escalas
materiales. El máximo de cada rama prueba su equilibrio uniforme; no
reproduce el switching de una geometría bidimensional ni la señal de un
fotón.}
\end{figure}

\textbf{Cálculo reejecutado en v0.2.} Se usaron \(T=0.9\) K,
\(T_c=8.65\) K, \(D=1.581\times10^{-4}\) m\(^2\)/s,
\(\sigma_n=4.2\times10^5\) S/m, \(w=120\) nm y \(d=7\) nm. Los máximos
son:
...
el método y los archivos que permiten reproducirlos. La coincidencia con
los valores publicados en v0.1 se establece ahora mediante una ejecución
...
\section{A.6. Material: por qué la corriente no fija toda la
energía}\label{a.6.-material-por-quuxe9-la-corriente-no-fija-toda-la-energuxeda}

\subsection{A.6.1. Acoplamiento débil y límite difusivo responden a
preguntas
distintas}\label{a.6.1.-acoplamiento-duxe9bil-y-luxedmite-difusivo-responden-a-preguntas-distintas}

El \textbf{límite difusivo} resulta de la dispersión elástica y de la
comparación de longitudes. El \textbf{acoplamiento débil} aproxima la
interacción de emparejamiento y conduce, entre otras relaciones, a
\(\Delta_0/(k_BT_c)\simeq1.764\). Un material puede ser difusivo y tener
...
figura 4.6 y texto, p.~impresa 64{]}. Ese valor se ha cotejado
documentalmente, sin recalcular aquí la integral material. La razón de
gap \(2.1k_BT_c\) citada en v0.1 se usa más abajo como contraste de
escalas, pendiente de justificar para la película concreta. Incorporar
\(\alpha^2F\) a las colisiones no convierte automáticamente el catálogo
BCS de equilibrio en un cálculo Eliashberg.

Como sensibilidad \textbf{algebraica}, sustituir una razón de gap 1.764
por 2.1 aumenta la escala de gap un 19.05\%, la escala de energía
\(N_0\Delta_0^2\) un 41.72\% y la escala de corriente
\(\sigma_n\Delta_0^{3/2}/\sqrt D\) un 29.89\%. No son correcciones
predichas para una película particular. Cambiar solo el número 1.764
manteniendo las mismas ecuaciones renormalizadas y el mismo \(T_c\)
tampoco es una solución Eliashberg: exige identificarlo como
aproximación empírica o construir un catálogo de acoplamiento fuerte
consistente.

\subsection{A.6.2. Dos combinaciones distintas de
parámetros}\label{a.6.2.-dos-combinaciones-distintas-de-paruxe1metros}
...
\subsection{A.6.3. Entradas que necesitan restricciones
materiales}\label{a.6.3.-entradas-que-necesitan-restricciones-materiales}
...
\section{A.7. Decisión de modelamiento para esta
iteración}\label{a.7.-decisiuxf3n-de-modelamiento-para-esta-iteraciuxf3n}

Se adopta como \textbf{referencia uniforme verificable} el funcional
Usadel de acoplamiento débil A.6 y sus fuerzas conjugadas. A.18 se
mantiene como una extensión no térmica adiabática propuesta, con
hipótesis explícitas. Esta combinación ofrece un camino para mejorar el
balance y los pesos espectrales sin descartar la justificación ni la
utilidad práctica del cierre anterior.
...
implementada en v0.2. Su necesidad se decidirá por la precisión
requerida y las restricciones materiales disponibles.
...
\href{https://arxiv.org/abs/2501.13791}{arXiv:2501.13791}. Referencia de
la cinética ampliada; los gráficos sintéticos de esta entrega no son
ejecuciones de sus datos de cascada.

{[}A{]} J. P. Allmaras, \emph{Modeling and Development of
Superconducting Nanowire Single-Photon Detectors}, tesis doctoral; J. P.
Allmaras et al., Physical Review Applied \textbf{11}, 034062 (2019).
\href{https://doi.org/10.1103/PhysRevApplied.11.034062}{DOI}.
Antecedentes del cierre dinámico, que debe compararse con la variante
concreta de la memoria.
...
\nolinkurl{391bb30}; referencia de amplitud:
\nolinkurl{pysnspd/gtdgl/material.py}; cálculo:
\nolinkurl{sandbox/model_v0_2/checks_a.py}. D registra los resultados y
la procedencia completa.
