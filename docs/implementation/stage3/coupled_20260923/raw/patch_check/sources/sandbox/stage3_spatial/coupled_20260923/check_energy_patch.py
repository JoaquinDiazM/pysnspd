...
def main():
    ...
    p.add_argument('--registration',default='docs/implementation/stage3/coupled_20260923/patch_registration.json')
    ...
    files=['pysnspd/experimental/'+n+'.py' for n in ['local_energy_patch','refined_cells','energy_catalog','spatial_functional']]
    files+=['sandbox/stage3_spatial/coupled_20260923/check_energy_patch.py',args.registration,reg['catalogue']]
    save(output/'manifest.json',dict(registration=reg,sources={f:sha(ROOT/f) for f in files}))
    progress=Progress(len(reg['degrees'])+len(reg['fractional_probes']),'Catálogo local')
    try:
        ...
        for degree in reg['degrees']:
            progress.start_task('construcción grado '+str(degree));begin=time.perf_counter()
            patch=LocalEnergyPatch.build(source,reg['amplitude_bounds'],reg['gamma_bounds'],degree,
