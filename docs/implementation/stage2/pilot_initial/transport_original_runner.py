...
def reference(cells,label,coefficient,order):
...
def main():
    ...
    registration={"schema":"pysnspd.stage2.transport-scenarios.v1","operator_sha256":sha(source),
        "catalog_sha256":sha(args.catalog),"criteria_sha256":sha(criteria),
        "fields":{name:FIELDS[name] for name in args.fields},"refinement_factors":args.refinement_factors,
        ...
        "dense_reference_scope":"Linear subdivisions of each native energy interval; positive count-control-volume weights used only for interpolation refinement. Native180 acceptance is evaluated independently.",
        "time_dynamics":False}
    ...
    for name in args.fields:
        ...
        exact.append({"field":name,**algebra(Operator(*base,gauss_order=4),base)})
        for factor in args.refinement_factors:
            cells=base if factor==1 else [DenseReferenceCell(c,factor) for c in base]
            refs={}
            for profile in ["thermal","nonthermal"]:
                low=reference(cells,profile,.03,24); high=reference(cells,profile,.03,48)
                refs[profile]=high
                ...
                for order in args.face_orders:
                    ...
                    rows.append({"field":name,"profile":profile,"electron_factor":factor,
    ...
    native=[r for r in rows if r["electron_factor"]==1 and r["face_order"]==max(args.face_orders)]
    result={"schema":"pysnspd.stage2.transport-review.v1","source_path":str(source),
        ...
        "native_status":"PASS" if all(r["passes_population_0_1_percent"] and r["passes_power_0_1_percent"] for r in native) else "FAIL",
        "native_max_relative_RHS_error":max(r["RHS_comparison"]["relative_weighted_L1"] for r in native),
        "native_max_relative_power_error":max(r["power_relative_error"] for r in native),
    ...
    print(json.dumps({key:result[key] for key in ["native_status","native_max_relative_RHS_error","native_max_relative_power_error","reference_max_relative_RHS_refinement","runtime_seconds"]},indent=2))
