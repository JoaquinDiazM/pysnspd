...
result=dict(status='PASS_PRESCRIBED_CIRCUIT_IDENTITY_ONLY',runtime_seconds=time.perf_counter()-started,
...
axes[0].plot(t*1e9,out*1e3,color='#007e80',lw=2,label='Circuito de la memoria')
axes[0].plot(t*1e9,RL*(I0-is_old)*1e3,color='#cf791b',ls='--',lw=1.8,label='Fuente ideal + carga')
axes[0].axhline(0,color='#666666',lw=.7);axes[0].set(xlabel='Tiempo (ns)',ylabel='Voltaje en carga (mV)',title='El capacitor modifica la cola de lectura')
