...
import time
import numpy as np
from continuous_reactions import ROOT,PROFILES,populations,sha,gapless_spectrum
...
def gapless_values(energy,a,g):
    """Reuse identical quadrature coordinates, never an operator spectrum."""
    unique,inverse=np.unique(energy,return_inverse=True)
    n,r,_=gapless_spectrum(unique,a,g)
    return n[inverse].reshape(energy.shape),r[inverse].reshape(energy.shape)


def point_rhs(energy,a,g,emax,order):
    ...
    gap=a if g==0 else 0.
    if g:
        ni,ri=gapless_values(energy,a,g)
        ratio=ri/ni
    ...
    for kind in ('upper','lower','recombination'):
        ...
        elif kind=='lower':lower=np.maximum(gap,energy-4.);upper=energy-.01
        else:lower=np.maximum(gap,.01-energy);upper=np.minimum(emax,4.-energy)
        valid=upper>lower
        xmin=np.sqrt(np.maximum(0.,lower*lower-a*a)) if g==0 else lower
        xmax=np.sqrt(np.maximum(0.,upper*upper-a*a)) if g==0 else upper
...
def weak_reference(grid,a,g,outer_order,inner_order):
    ...
    low=np.maximum(grid[:-1],a if g==0 else 0.);high=grid[1:]
    valid=high>low
    ...
    e=np.hypot(a,x) if g==0 else x
    if g:
        ni,ri=gapless_values(e,a,g)
        measure=measure*ni
    ids=np.flatnonzero(valid)
    beta=(e-grid[ids,None])/(grid[ids+1,None]-grid[ids,None])
    rhs=point_rhs(e.ravel(),a,g,float(grid[-1]),inner_order).reshape(3,2,len(ids),outer_order)
...
def main():
    ...
    parser.add_argument('--field',choices=('normal','bcs','gapless','all'),default='all')
    ...
    rows=[]
    fields={'normal':(0.,0.),'bcs':(.72,0.),'gapless':(.35,.65)}
    selected=list(fields.values()) if args.field=='all' else [fields[args.field]]
    for a,g in selected:
        grid=cat.energy_kernel(a,g)[0]
        levels=[]
        for outer,inner in ((2,16),(4,32),(8,64)):
            tick=time.perf_counter();values=weak_reference(grid,a,g,outer,inner)
            levels.append(values)
            print(json.dumps(dict(amplitude=a,gamma=g,outer=outer,inner=inner,seconds=time.perf_counter()-tick)),flush=True)
        for ip,profile in enumerate(PROFILES):
            channels={}
            ...
            rows.append(dict(amplitude=a,gamma=g,profile=profile,energy_nodes=grid.tolist(),channels=channels))
    data=dict(schema='pysnspd.stage2.independent_weak_population.v1',
        status='IDEAL_CONTINUOUS_WEAK_REFERENCE',
        ...
        fields=selected,count_refinement=args.refinement,electron_states=len(cat.count_nodes),
    ...
    target=args.output or out/f'weak_population_reference_edge8_{args.field}_{args.refinement}.json'
